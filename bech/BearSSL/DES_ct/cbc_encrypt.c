#include "../../ct-verif.h"
#include "inner.h"
void cbc_encrypt_wrapper(const br_des_ct_cbcenc_keys *ctx,
	...
	br_des_ct_cbcenc_run(ctx, iv, data, len);
}


const br_des_ct_cbcenc_keys *getctx();
void *getpt2();
void *getpt3();
unsigned getuns();


void vfct_tmp(uint32_t* skey, const br_des_ct_cbcenc_keys *ctx, void *iv, void *data, size_t len){
	br_des_ct_cbcdec_run(ctx, iv, data, len);
}


void cbc_encrypt_wrapper_t(){
	const br_des_ct_cbcenc_keys *ctx = getctx();
	void *iv = getpt2();
	void *data = getpt3();
	 size_t len = 32;
