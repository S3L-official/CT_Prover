...
$bb1:
  ...
  assume {:VFCTMarked} true;
  assert {:likely_shadow_invariant} ($i32 == $i32.shadow);
  assume {:VFCTMarked} true;
  assert {:likely_shadow_invariant} ($p33 == $p33.shadow);
  assume {:VFCTMarked} true;
...
$bb7:
  ...
  assert {:likely_shadow_invariant} ($p7 == $p7.shadow);
  assume {:VFCTMarked} true;
  assert {:likely_shadow_invariant} ($i149 == $i149.shadow);
  assume {:VFCTMarked} true;
  assert {:likely_shadow_invariant} ($p8 == $p8.shadow);
  assume {:VFCTMarked} true;
  assert {:likely_shadow_invariant} ($i38 == $i38.shadow);
  assume {:VFCTMarked} true;
  assert {:likely_shadow_invariant} ($p39 == $p39.shadow);
  assume {:VFCTMarked} true;
  assert {:likely_shadow_invariant} ($p6 == $p6.shadow);
  assume {:sourceloc "../BearSSL/src/hash/ghash_ctmul.c", 0, 0} true;
  assume {:verifier.code 0} true;
