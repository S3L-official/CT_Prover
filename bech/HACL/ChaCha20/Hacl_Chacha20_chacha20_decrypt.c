...
#include "../../ct-verif.h"
void Hacl_Chacha20_chacha20_decrypt_wrapper(uint32_t len,
  uint8_t *out,
  uint8_t *cipher,
  uint8_t *key,
  uint8_t *n,
  uint32_t ctr)
{
  ...
  public_in(__SMACK_values(cipher,64));
  //n是初始向量IV
  Hacl_Chacha20_chacha20_decrypt(len, out,cipher,key,n,ctr);
}
...
uint8_t *getpt4();
uint32_t getnum();
void Hacl_Chacha20_chacha20_decrypt_wrapper_t(){
	uint8_t* out = getpt1();
