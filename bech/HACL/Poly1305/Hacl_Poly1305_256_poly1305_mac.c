...
#include "../../ct-verif.h"
void Hacl_Poly1305_256_poly1305_mac_wrapper(uint8_t *tag, uint32_t len, uint8_t *text, uint8_t *key)
{
  ...
  public_in(__SMACK_values(text,16));
  //n是初始向量IV
...
void Hacl_Poly1305_256_poly1305_mac_wrapper_t(){
	uint8_t* tag = getpt1();
	uint8_t* text = getpt2();
  uint8_t* key = getpt3();
  uint8_t len = getnum2();
