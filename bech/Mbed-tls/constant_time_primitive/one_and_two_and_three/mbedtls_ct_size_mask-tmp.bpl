...
const mbedtls_ct_size_mask_wrapper: ref;
axiom (mbedtls_ct_size_mask_wrapper == $sub.ref(0,3096));
procedure {:entrypoint} mbedtls_ct_size_mask_wrapper($i0: i64, $i0.shadow: i64) returns ($r: i32, $r.shadow: i32)
modifies $exn;
modifies $CurrAddr;
{
  var $i1: i32;
...
$bb0:
  $shadow_ok := true;
  call $initialize();
  assume {:sourceloc "mbedtls_ct_size_mask.c", 10, 2} true;
  assume {:verifier.code 0} true;
  call {:cexpr "smack:entry:mbedtls_ct_size_mask_wrapper"} boogie_si_record_ref(mbedtls_ct_size_mask_wrapper);
  call {:cexpr "smack:arg:mbedtls_ct_size_mask_wrapper:$i0"} boogie_si_record_i64($i0);
  call {:cexpr "mbedtls_ct_size_mask_wrapper:arg:value"} boogie_si_record_i64($i0);
  assume {:sourceloc "mbedtls_ct_size_mask.c", 10, 2} true;
  assume {:verifier.code 0} true;
  call $i1, $i1.shadow := mbedtls_ct_size_mask($i0, $i0.shadow);
  assume {:sourceloc "mbedtls_ct_size_mask.c", 12, 1} true;
  assume {:verifier.code 0} true;
  $r := $u0;
  $r.shadow := $r;
  $exn := false;
