...
int curve25519_donna_wrapper_t() {
  u8 mmypublic = getvalue();
  u8 *mypublic = &mmypublic;
  const u8 nsecret = getvalue2();
  const u8 *secret = &nsecret;
  const u8 nbasepoint = getvalue3();
  const u8 *basepoint = &nbasepoint;
