...
#include "ct-verif.h"
void en_de_wrapper(const byte *key_1, const byte *plaintext_1, const byte ciphertext_1, ARCFOUR_context ctx,   byte *scratch){
    
  /* all pointer value are public */
  public_in(__SMACK_value(key_1));
  public_in(__SMACK_value(plaintext_1));
  public_in(__SMACK_value(ciphertext_1));
  public_in(__SMACK_value(scratch));

  public_in(__SMACK_values(plaintext_1, 32));
  public_in(__SMACK_values(scratch, 32));
