...
/// can then be queried by using resultAt() and resultsAt().
    ...
    public:
        ...
        virtual void solve() {
            ...
            submitInitialSeeds();
//      auto tt = clock();
//      llvm::errs()<<(double)(tt)/CLOCKS_PER_SEC<<"  "<<"End of phaseI"<<"\n";
        ...
        ///VFCT
/*    std::vector<psr::Table<llvm::Instruction const*,llvm::Value const*,psr::BinaryDomain>::Cell> getdumpresult(){
        auto cells = this->ValTab.cellVec();
        if (cells.empty()) {
...
//            PHASAR_LOG_LEVEL(DEBUG,"propagateValueAtStart");
            PAMM_GET_INSTANCE;
...
namespace psr {
    ...
    public:
        ...
        LuweiIDESolver(IDETabulationProblem<AnalysisDomainTy, Container> &Problem)
                : IDEProblem(Problem), ZeroValue(Problem.getZeroValue()),
                  ICF(Problem.getICFG()), SolverConfig(Problem.getIFDSIDESolverConfig()),
                  CachedFlowEdgeFunctions(Problem), AllTop(Problem.allTopFunction()),
                  JumpFn(std::make_shared<JumpFunctions<AnalysisDomainTy, Container>>(
        ...
        LuweiIDESolver(const LuweiIDESolver &) = delete;
        LuweiIDESolver &operator=(const LuweiIDESolver &) = delete;
        LuweiIDESolver(LuweiIDESolver &&) = delete;
        LuweiIDESolver &operator=(LuweiIDESolver &&) = delete;

        virtual ~LuweiIDESolver() = default;

        struct pathRecord{
        public:
            pathRecord(const PathEdge<n_t, d_t>& edge,const EdgeFunctionPtrType& f) : edge(edge),f(f){}
            pathRecord(const pathRecord& r) : edge(r.edge), f(r.f){}
            pathRecord(pathRecord&& r) : edge(r.edge), f(r.f){}
//                pathRecord(pathRecord& r) = default;
            PathEdge<n_t, d_t> edge;
            const EdgeFunctionPtrType f;
        };
        ...
        bool findvalue(n_t Stmt, d_t Value){
            std::unordered_map<d_t, l_t> &Result = ValTab.row(Stmt);
            auto fit = Result.find(Value);
            if(fit != Result.end()){
                // need to consider again
//                if(fit->second == l_t::BOTTOM)
                    return true;
            }
            return false;
        }

        auto &getvalues(n_t Stmt){
            std::unordered_map<d_t, l_t> &Result = ValTab.row(Stmt);
            return Result;
        }

        bool
        resultAtInSparsePropgate(n_t Stmt, d_t Value) {
            if (Stmt->getType()->isVoidTy()) {
                return findvalue(Stmt, Value);
            }
            if(!Value.isPointerTy()){
                auto users = Value.users();
                for(auto use : users){
                    auto st = llvm::dyn_cast<llvm::Instruction>(use);
                    if(auto callins = llvm::dyn_cast<llvm::CallInst>(use))
                        return findvalue(*(ICF->getReturnSitesOfCallAt(callins).begin()), Value);
                    return findvalue(st, Value);
                    //寻找一个非call的user,如果找不到则在后续的逻辑中寻找后继结点找
                    //没有user也会进入到后续的逻辑中
//                    auto callins = llvm::dyn_cast<llvm::CallInst>(use);
//                    if(callins == nullptr){
//                        auto resstmt = llvm::dyn_cast<llvm::Instruction>(use);
//                        return findvalue(resstmt, Value);
//                    }
                }
            }
            if (!Stmt->getNextNode()) {
            ...
            assert(Stmt->getNextNode() && "Expected to find a valid successor node!");
            //只有声明的指令不能很好的传递
            auto nxt = Stmt->getNextNonDebugInstruction();
//            while(llvm::isa<llvm::CallInst>(nxt) && nxt->getType()->isVoidTy()){
//                nxt = nxt->getNextNonDebugInstruction();
//            }
            return findvalue(nxt, Value);
        }
...
/*    std::vector<psr::Table<llvm::Instruction const*,llvm::Value const*,psr::BinaryDomain>::Cell> getdumpresult(){
    ...
    protected:
        ...
        std::vector<std::pair<PathEdge<n_t, d_t>, EdgeFunctionPtrType>> WorkList;
        ...
        template <typename IFDSAnalysisDomainTy,
                typename = std::enable_if_t<
                        is_analysis_domain_extensions<AnalysisDomainTy>::value,
                        IFDSAnalysisDomainTy>>
        LuweiIDESolver(IFDSTabulationProblem<IFDSAnalysisDomainTy, Container> &Problem)
                : IFDSExtension<AnalysisDomainTy, Container>(Problem),
                  IDEProblem(*this->TransformedProblem),
                  ZeroValue(IDEProblem.getZeroValue()), ICF(IDEProblem.getICFG()),
                  SolverConfig(IDEProblem.getIFDSIDESolverConfig()),
                ...
                if (SpecialSum) {
                    ...
                    for (n_t SP : StartPointsOf) {
                        ...
                        for (d_t d3 : Res) {
                            ...
                            PHASAR_LOG_LEVEL(DEBUG, "Create initial self-loop with D: "
                                    << IDEProblem.DtoString(d3));
                            WorkList.emplace_back(PathEdge(d3, SP, d3),EdgeIdentity<l_t>::getInstance()); // line 15
                            // register the fact that <sp,d3> has an incoming edge from <n,d2>
                            // line 15.1 of Naeem/Lhotak/Rodriguez
                            addIncoming(SP, d3, n, d2);
                            // line 15.2, copy to avoid concurrent modification exceptions by
                            ...
                            for (const TableCell &Entry : endSummary(SP, d3)) {
                                ...
                                for (n_t RetSiteN : ReturnSiteNs) {
                                    ...
                                    for (d_t d5 : ReturnedFacts) {
                                        ...
                                        PHASAR_LOG_LEVEL(DEBUG, "Compose: " << fPrime->str() << " * "
                                                                            << f->str());
                                        WorkList.emplace_back(PathEdge(d1, RetSiteN, std::move(d5_restoredCtx)),
            ...
            for (n_t ReturnSiteN : ReturnSiteNs) {
                ...
                for (d_t d3 : ReturnFacts) {
                    ...
                    PHASAR_LOG_LEVEL(DEBUG, "Compose: " << EdgeFnE->str() << " * "
                                                        << f->str() << " = "
                                                        << fPrime->str());
                    WorkList.emplace_back(PathEdge(d1, ReturnSiteN, std::move(d3)),fPrime);
                }
            }
        }
            ...
            for (const auto nPrime : ICF->getSuccsOf(n)) {
                ...
                for (d_t d3 : Res) {
                    ...
                    if (!d3.isPointerTy()){
                        auto users = d3.users();
                        for(auto U : users){
                            if(auto callins = llvm::dyn_cast<llvm::CallInst>(U)){
                                if(auto calledfun = callins->getCalledFunction()){
                                    if(callins->getCalledFunction()->getName().equals("taint_declassify")){
                                        flg = true;
                                        break;
                                    }
                                }
                        ...
                        if(auto callins = llvm::dyn_cast<llvm::CallInst>(nPrime)){
                            if(callins->getName().equals("taint_declassify")){
                                auto numofactarg = callins->getNumOperands();
                                for(unsigned i = 0; i < numofactarg; i++){
                                    auto actpara = callins->getArgOperand(i);
                                    if(d3 == actpara){
                                        flg = true;
                                        break;
                                    }
                    ...
                    if(d3.isPointerTy()){
                        ...
                        PHASAR_LOG_LEVEL(DEBUG, "Compose: " << g->str() << " * " << f->str()
                                                            << " = " << fPrime->str());
                        INC_COUNTER("EF Queries", 1, PAMM_SEVERITY_LEVEL::Full);
                        WorkList.emplace_back(PathEdge(d1, nPrime, std::move(d3)),fPrime);
                    }else{
                        auto users = d3.users();
                        bool hasuser = false;
                        for(auto U : users){
                            ...
                            hasuser = true;
                            auto I = dyn_cast<llvm::Instruction>(U);
                            if(!I){
                                assert(0 && "Some Users are not Instruction!");
                            }
                            EdgeFunctionPtrType g =
                                    CachedFlowEdgeFunctions.getNormalEdgeFunction(n, d2, I, d3);
                            PHASAR_LOG_LEVEL(DEBUG, "Queried Normal Edge Function: " << g->str());
                            EdgeFunctionPtrType fPrime = f->composeWith(g);
                            if (SolverConfig.emitESG()) {
                                IntermediateEdgeFunctions[std::make_tuple(n, d2, I, d3)]
                                        .push_back(g);
                            }
                            PHASAR_LOG_LEVEL(DEBUG, "Compose: " << g->str() << " * " << f->str()
                                                                << " = " << fPrime->str());
                            INC_COUNTER("EF Queries", 1, PAMM_SEVERITY_LEVEL::Full);
                            WorkList.emplace_back(PathEdge(d1, I, std::move(d3)),fPrime);
                        }
                        //如果一个user都没有，传递到下一个指令用以做标记
//                        if(!hasuser){
//                            EdgeFunctionPtrType g =
//                                    CachedFlowEdgeFunctions.getNormalEdgeFunction(n, d2, nPrime, d3);
//                            PHASAR_LOG_LEVEL(DEBUG, "Queried Normal Edge Function: " << g->str());
...
//                        }
        ...
        void propagateValue(n_t NHashN, d_t NHashD, const l_t &L) {
            PHASAR_LOG_LEVEL(DEBUG,
                             "propagate   value: N " << IDEProblem.NtoString(NHashN) << " D "<<IDEProblem.DtoString(NHashD)<< " L "<< IDEProblem.LtoString(L));
            l_t ValNHash = val(NHashN, NHashD);
            l_t LPrime = joinValueAt(NHashN, NHashD, ValNHash, L);
            PHASAR_LOG_LEVEL(DEBUG,"ValNHash: "<< IDEProblem.LtoString(ValNHash)<<" LPrime "<<IDEProblem.LtoString(LPrime));
            if (!(LPrime == ValNHash)) {
                setVal(NHashN, NHashD, std::move(LPrime));
                valuePropagationTask(std::pair<n_t, d_t>(NHashN, NHashD));
            }
        ...
        l_t val(n_t NHashN, d_t NHashD) {
            if (ValTab.contains(NHashN, NHashD)) {
                PHASAR_LOG_LEVEL(DEBUG,"if");
                return ValTab.get(NHashN, NHashD);
            }
            PHASAR_LOG_LEVEL(DEBUG,"else");
            // implicitly initialized to top; see line [1] of Fig. 7 in SRH96 paper
            return IDEProblem.topElement();
        }
        ...
        void valuePropagationTask(const std::pair<n_t, d_t> NAndD) {
            ...
            // should be treated as such the same also for unbalanced return sites in
            // an unbalanced problem
            PHASAR_LOG_LEVEL(DEBUG,"valuePropagationTask " << IDEProblem.NtoString(n));
            if (ICF->isStartPoint(n) || Seeds.containsInitialSeedsFor(n) ||
                UnbalancedRetSites.count(n)) {
                PHASAR_LOG_LEVEL(DEBUG,"valuePropagationTask " << " is start");
                // FIXME: is currently not executed for main!!!
                // initial seeds are set in the global constructor, and main is also not
                // officially called by any other function
                propagateValueAtStart(NAndD, n);
            ...
            if (ICF->isCallSite(n)) {
                PHASAR_LOG_LEVEL(DEBUG,"valuePropagationTask " << " is callsite");
                propagateValueAtCall(NAndD, n);
            }
        }
        ...
        void submitInitialSeeds() {
            ...
            for (const auto &[StartPoint, Facts] : Seeds.getSeeds()) {
                for (const auto &[Fact, Value] : Facts) {
                    ...
                    WorkList.emplace_back(PathEdge(Fact, StartPoint, Fact), EdgeIdentity<l_t>::getInstance());
                }
            }
        ...
        void propagteUnbalancedReturnFlow(n_t RetSiteC, d_t TargetVal,
                                          EdgeFunctionPtrType EdgeFunc,
                                          n_t RelatedCallSite) {
            WorkList.emplace_back(PathEdge(ZeroValue, std::move(RetSiteC),
                                           std::move(TargetVal)),EdgeFunc);
        }
        ...
        void propagate(d_t SourceVal, n_t Target, d_t TargetVal,
            ...
            if (NewFunction) {
                JumpFn->addFunction(SourceVal, Target, TargetVal, fPrime);
                PathEdge<n_t, d_t> Edge(SourceVal, Target, TargetVal);
                PathEdgeCount++;
