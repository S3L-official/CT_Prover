  ...
  for (const auto &DFA : DataFlowAnalyses) {
    ...
    if (std::holds_alternative<DataFlowAnalysisType>(DFA)) {
      ...
      case DataFlowAnalysisType::IFDSTaintAnalysis: {
          ...
          WholeProgramAnalysis<LuweiIFDSSolver_P<IFDSTaintAnalysis>, IFDSTaintAnalysis>
          WPA(SolverConfig, IRDB, &Config, EntryPoints, &PT, &ICF, &TH);
          WPA.solve();
          emitRequestedDataFlowResults(WPA);

          std::set<llvm::Instruction*> res;
          std::set<llvm::Instruction*> res2;
          std::set<llvm::Instruction*> res3;
          WPA.getdumpresult(res,res2,res3);
          Pitch pt;
          pt.run(res,res2,res3,WPA.getTaintedCalled(), IRDB.getAllModules(), EntryPoints);
          // 不要删掉下面两行，删掉会有释放内存时的错误。
...
//        clock_t  tt3 = clock();
//        llvm::errs()<<"End of If"<<"\n";
//        llvm::errs()<<(double)(tt3)/CLOCKS_PER_SEC<<"\n";
...
//      clock_t  tt2 = clock();
//      llvm::errs()<<"End of one for"<<"\n";
//      llvm::errs()<<(double)(tt2)/CLOCKS_PER_SEC<<"\n";
  }
//    clock_t  tt = clock();
//    llvm::errs()<<(double)(tt)/CLOCKS_PER_SEC<<"\n";
}
