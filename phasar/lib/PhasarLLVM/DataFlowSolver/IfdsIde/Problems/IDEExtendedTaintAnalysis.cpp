...
用来测试在ide中alias analysis结果的代码
        return makeLambdaFlow<d_t>([this, Load](d_t Source) -> std::set<d_t> {
            llvm::errs()<< *Load<< "    "<<*Load->getPointerOperand()<<"      " << *(Source->base())<<"\n";
            llvm::errs()<<toString(PT->alias(Load->getPointerOperand(),Source->base()));
            llvm::errs()<<"\n\n";
            return {Source};
        });
    }
