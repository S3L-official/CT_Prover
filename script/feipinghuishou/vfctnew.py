...
def buildbech(wrapper, lib, entryP, headfilepath = " "):
    args = "buildbech " + wrapper + lib + entryP + headfilepath
    restime = runcommand(args)
    return restime


#spbuild 生成的文件名固定是total-opt.ll
#只需要编译wrapper源文件，link lib文件即可
def clang(files):
    global libs 
    if libs != "":
        files = libs + " " + files
...
def verifybpl(file, resfile):
    with open(resfile, "w") as outfile:   
        args = "boogie /doModSetAnalysis /errorLimit 100 /timeLimit 21600 "+file
