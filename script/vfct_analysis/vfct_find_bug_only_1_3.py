...
def one_and_two_and_three():
    ...
    for item in entry:
        ...
        res = [restime1+restime2, restime3+restime4+t6]
        ress.append(res)
...
def collect_time():
    df=pd.DataFrame(ress,index=entry, columns=['taint-analysis','generatebpl'])
    df.to_csv("./one_and_two_and_three/generate.csv")
    # df=pd.DataFrame(shadowress,index=entry, columns=['shadow-generatebpl'])
