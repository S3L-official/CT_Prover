...
def chaifen_single_two():
    ...
    for item in entry:
        ...
        restime3 = specialverifybpl(boolboogiefile, boogieres,"chaifen_single_two")
...
def chaifen_single_three_0():
    mkdir("chaifen_single_three_0")
    sigtwo1 = []
    sigtwo2 = []
    sigtwo3 = []
    total = []
    for item in entry:
        ...
        boogiefile = item + "-shadow.bpl"
        boogieres =  item + "-shadow.txt"
        restime1 = generatebpl(file, boogieentry, outfile, "chaifen_single_three_0")
        restime2 = productbpl(outfile, boogiefile, NoTaint, ShadowProduct, SplitAssert,"chaifen_single_three_0")
        restime3 = specialverifybpl(boogiefile, boogieres,"chaifen_single_three_0")
    ...
    totaltime_3_0.append(("3-0", total))
...
def markbpl(restxt, bpl, dir):
    args = "MarkLoopInvariant.py "+ restxt +" " + bpl
    restime = runcommand(args, workdir=dir)
    return restime


def newtransfer(record1,record2, file,file2, recordfile = subprocess.PIPE, workdir = os.getcwd()):
    # 需要配合bam991 一起使用
    args = "transBoolToShadow2.py "+record1+" "+record2+" "+file
    restime = runcommand(args, recordfile, workdir = workdir)
    args = "ruby -I /home/luwei/bam-991/lib /home/luwei/bam-991/bin/bam --process_mark "+ file + " -o " + file2
    restime2 = runcommand(args, workdir=workdir)
    return restime, restime2

def one_and_two_and_three_1():
    dirname = "one_and_two_and_three_1"
    mkdir(dirname)
    total = []
    total2 = []
    l1 = []
    ...
    for item in entry:
        ...
        boogiefile = item + ".bpl"
        boolboogiefile = item + "-bool.bpl"
        boolboogiefilenoloop = item + "-bool_Noloop.bpl"
        tmpboogiefile = item + "-tmp.bpl"
        shadowboogiefile = item + "-shadow.bpl"
        analyres = item + "-1-2.txt"
        analyres2 = item + "-1-22.txt"
        boogieentry = item + "_wrapper"
        trasnsres = item + "-trans.txt"
        finalres = item  + "-1-2-3.txt"

        runcommand("cp " + taintconfig+ " " + dirname)

        restime1 = addkey(file, tkfile, dirname)
        restime2 = phasar(tkfile, taintentry, taintconfig, outfile, dirname)
       
        restime3 = generatebpl(taintfile, boogieentry, boogiefile, dirname)
        restime4 = productbpl(boogiefile, boolboogiefile, AcceptTaint, BoolProduct, SplitAssert,dirname)
        restime5 = specialverifybpl(boolboogiefile, analyres,dirname)

        t6 = markbpl(analyres, boolboogiefile, dirname)
        t7 = specialverifybpl(boolboogiefilenoloop,analyres2 ,dirname)
        t8 = productbpl(boogiefile, tmpboogiefile, AcceptTaint, ShadowProduct, NoSplitAssert, dirname)
        t9, t10 = newtransfer(analyres, analyres2, tmpboogiefile, shadowboogiefile, trasnsres, dirname)

        t11 = verifybpl(shadowboogiefile, finalres, dirname)
        ...
        # t6 = productbpl(boogiefile, tmpboogiefile, AcceptTaint, ShadowProduct, NoSplitAssert, dirname)
        # t7, t8 = transfer(analyres, tmpboogiefile, shadowboogiefile, trasnsres, dirname)
        # t9 = verifybpl(shadowboogiefile, finalres, dirname)

        # high_taint_res_1_2_3.append(get_high_taint_res("one_and_two_and_three/"+trasnsres)[1])    

        l1.append(restime1+restime2)
        l2.append(restime3+restime3+restime5)
        l3.append(t6+t7)
        l4.append(t8+t9+t10+t11)

        total.append(restime1+ restime2 + restime3 + restime4 + restime5 + t6 + t7 + t8 + t9)


    detailtime_1_2_3_1.append(("1", l1))
    detailtime_1_2_3_1.append(("2", l2))
    detailtime_1_2_3_1.append(("2-2", l3))
    detailtime_1_2_3_1.append(("3", l4))
    totaltime_1_2_3_1.append(("1+2+3", total))
...
def one_and_two_and_three_2():
    ...
    totaltime_1_2_3.append(("1+2+3", total))
    # totaltime2_1_2_3.append(("1+2+3", total2))
    # detailtime_1_2_3.append(("1+2+3-addkey", l1))
    # detailtime_1_2_3.append(("1+2+3-taint-analysis", l2))
    # detailtime_1_2_3.append(("1+2+3-generatebpl", l3))
...
def runall():
    ...
    # threads.append(t5)
    # t6 = threading.Thread(target=chaifen_single_three_0)
    # threads.append(t6)
    ...
    # threads.append(t8)
    t9 = threading.Thread(target=one_and_two_and_three_1)
    threads.append(t9)
    # t10 = threading.Thread(target=one_and_two_and_three_s)
    # threads.append(t10)
    # t11 = threading.Thread(target=one_and_two_and_three_0)
    # threads.append(t11)
    ...
    for i in threads:
        ...
        #收集时间
    global totaltime
    totaltime = totaltime_1_2_3_1
    global detailtime
    detailtime = detailtime_1_2_3_1
