...
void SmackInstGenerator::annotate(llvm::Instruction &I, Block *B) {
  ...
  if(I.getMetadata("ValueTainted")){
      B->addStmt(Stmt::annot(Attr::attr("ValueTainted")));
  }

  if(I.getMetadata("PointTainted")){
      B->addStmt(Stmt::annot(Attr::attr("PointTainted")));
  }

  if(I.getMetadata("Tainted")){
      B->addStmt(Stmt::annot(Attr::attr("Tainted")));
  }
...
//  }
//
  if(I.getMetadata("CalledTainted")){
      auto MNode = I.getMetadata("CalledTainted");
      std::string ss(cast< MDString >(MNode->getOperand(0))->getString());
      B->addStmt(Stmt::annot(Attr::attr("CalledTainted",ss)));
  }
  ...
  for (unsigned i = 0; i < ti.getNumSuccessors(); i++) {
    ...
    for (llvm::BasicBlock::iterator s = ti.getSuccessor(i)->begin(),
      ...
      llvm::PHINode *phi = llvm::cast<llvm::PHINode>(s);
        ...
        if(phi->getMetadata("ValueTainted")){
            flg.push_back(true);
        }else if(phi->getMetadata("Tainted")){
            flg.push_back(true);
        }else if(phi->getMetadata("PointTainted")){
            flg.push_back(true);
        }else
            flg.push_back(false);
...
//    emit(Stmt::assign(lhs, rhs));
        auto i = 0;
      for (auto it = lhs.begin(), it2 = rhs.begin(); it != lhs.end(); ++it, it2++, i++) {
          if(flg[i]){
              emit(Stmt::annot(Attr::attr("ValueTainted")));
          }
          emit(Stmt::assign(*it, *it2));
      }
  }
}
